 ...
 * Features: Distribution dots, Species sprites, Ocean currents, Fly-to + sliding card
 */
...
function createCurrentAnimator(loader: any, earthGroup: any): any {
...
function SpeciesCardPanel({ data, onClose }: { data: CardData; onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      zIndex: 200, willChange: 'transform, opacity',
    }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', fontSize: '12px', marginBottom: '10px' }}>
        {[['保护状态', iucn, iucnColor], ['最大体长', maxLen, '#ddd'], ['温度带', `${zoneEmoji} ${zoneLabel}`, '#ddd'], ['食性', diet ? diet.substring(0, 6) : '—', '#ddd']].map(([label, val, color], i) => (
...
export default function Globe3D({ distributions = [], species = [], speciesImages = {} }: Props) {
  ...
  const dismissCard = useCallback(() => {
    ...
    if (!sr.scene) return
    if (sr.activeLabelObj) { sr.scene.remove(sr.activeLabelObj); sr.activeLabelObj = null }

    const { earthGroup, camera, controls } = sr
    if (earthGroup) {
      new TWEEN.Tween(earthGroup.scale).to({ x: 1, y: 1, z: 1 }, 600).easing(TWEEN.Easing.Cubic.InOut).start()
      new TWEEN.Tween(earthGroup.position).to({ x: 0, y: 0, z: 0 }, 600).easing(TWEEN.Easing.Cubic.InOut).start()
    }
    if (camera && controls) {
  ...
  const toggle = useCallback((i: number) => {
    ...
    const sr = stateRef.current
    if (i === 0 && sr.worldDots)   sr.worldDots.visible = next[0]
    if (i === 1) {
      // Species sprites: visible when checked OR when card is shown
      sr.spriteMarkers?.forEach((s: any) => { s.visible = next[1] || cardVisible })
    }
    if (i === 2 && sr.currAnim) {
      sr.currAnim.particles.forEach((p: any) => { p.visible = next[2] })
    }
  }, [cardVisible])
  ...
  useEffect(() => {
    ...
    // Earth group (all geo inside it → unified shrink/translate)
    const earthGroup = new THREE.Group()
    scene.add(earthGroup)

    // CSS2DRenderer (for 3D-attached label, currently unused but kept for future)
    ...
    // ── Species sprites (near-field) ─────────────────────────────────────
    let spriteMarkers: any[] = []

    // ── Ocean currents ─────────────────────────────────────────────────────
    let currAnim: any = null
    ...
    const buildMarkers = (distList: SpeciesDistribution[]) => {
      ...
      distList.forEach((d: any) => {
        if (seen.has(d.species_id)) return
        seen.add(d.species_id)
        if (!speciesDistMap.current[d.species_id]) speciesDistMap.current[d.species_id] = d
        ...
        sprite.visible = false  // controlled by toggle
    ...
    // Build markers + ocean currents
    if (distributions.length > 0) buildMarkers(distributions)
    currAnim = createCurrentAnimator(loader, earthGroup)
    // Apply current visibility state
    currAnim.particles.forEach((p: any) => { p.visible = checkboxRef.current[2] })
    ...
    stateRef.current = {
      ...
      worldDots, spriteMarkers, currAnim,
      buildMarkers, activeLabelObj,
    }
    ...
    const animate = (time: number) => {
      ...
      // LOD: when close enough, show sprites instead of dots
      if (worldDots) {
        const showSprites = dist < 42
        worldDots.visible = !showSprites && checkboxRef.current[0]
        spriteMarkers.forEach(s => {
          s.visible = (showSprites || checkboxRef.current[1]) && !cardVisible
        })

        // Lazy load sprite textures
        if (showSprites) {
